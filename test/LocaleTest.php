...
class LocaleTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertInstanceOf(PhpSourceLoader::class, (new Locale([
            'loader' => PhpSourceLoader::class
        ]))->getLoader());
        ...
        $this->assertInstanceOf(PhpSourceLoader::class, (new Locale([
            'loader' => new PhpSourceLoader
        ]))->getLoader());
    }

    public function testLoaderSetLanguage()
    {
        $l = new Locale([
            'loader' => PhpSourceLoader::class
        ]);
        $l->getLoader()->setLanguage('ru');
        $this->assertEquals('ru', $l->getLoader()->getLanguage());
    }
    ...
    public function testRequiredDataExistence()
    {
        ...
        foreach ($this->criticalLocaleCodes as $localeCode) {
            $locale = $t->getLocaleData($localeCode);
            // AM/PM
    ...
    {
        $locale = (new Locale)->getLocaleData($ctorLocale);
